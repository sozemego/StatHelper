...
const experimentalDesign = (state = initialState, action) => {
    switch (action.type) {
        ...
        case START_SELECTING_SCALES: return { ...state, selectingItems: true };
        case MOUSE_UP: return { ...state, selectingItems: false };
        case SELECT_SCALES: return { ...state, tests: selectScales(state.tests, action.testIndex, action.selectedScales)};
